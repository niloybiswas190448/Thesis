#!/usr/bin/env python3
"""
Enhanced Dhaka Public Transport Fare System Optimization Analysis (No NumPy)
===========================================================================
...
This version uses only standard Python libraries (no numpy required).
"""
...
class EnhancedThesisAnalyzer:
    ...
    def objective_1_analyze_user_preferences_and_affordability(self):
        ...
        for income_group in self.income_groups:
            ...
            wtp_results[income_group] = {
                'mean_wtp': self._calculate_mean(wtp_values),
                'median_wtp': self._calculate_median(wtp_values),
                'min_wtp': min(wtp_values),
                'max_wtp': max(wtp_values),
                'mean_affordability_ratio': self._calculate_mean(affordability_ratios),
    ...
    def _perform_discrete_choice_modeling(self):
        ...
        for income_group in self.income_groups:
            ...
            for mode in self.transport_modes:
                ...
                if mode_data:
                    avg_fare = self._calculate_mean([row['Fare_Paid'] for row in mode_data])
                    avg_time = self._calculate_mean([row['total_time'] for row in mode_data])
                    avg_comfort = self._calculate_mean([row['Comfort_Level'] for row in mode_data])
    ...
    def _calculate_fare_elasticity(self, group_data):
        ...
        mean_fare = self._calculate_mean(fares)
        mean_demand = self._calculate_mean(demands)
        ...
        for f, d in zip(fare_changes, demand_changes):
            if f != 0:
                elasticity_components.append((d/mean_demand) / (f/mean_fare))
        
        if not elasticity_components:
            return 0.0
        
        elasticity = self._calculate_mean(elasticity_components)
        return abs(elasticity) if not math.isnan(elasticity) else 0.0
    ...
    def _perform_cvm_analysis(self):
        ...
        for income_group in self.income_groups:
            ...
            avg_wtp_fare_improvement = self._calculate_mean([row['WTP'] for row in wtp_lower_fare]) if wtp_lower_fare else 0
            avg_wtp_comfort_improvement = self._calculate_mean([row['WTP'] for row in wtp_higher_comfort]) if wtp_higher_comfort else 0
    ...
    def objective_2_model_social_welfare_outcomes(self):
        ...
        for mode in self.transport_modes:
            ...
            base_price = self._calculate_mean([row['Fare_Paid'] for row in mode_data])
    ...
    def _run_welfare_optimization_models(self, mode, base_demand, base_price, base_cost):
        ...
        price_range = [base_price * (0.5 + i * 0.015) for i in range(100)]
        
        # Max Revenue
        revenues = [revenue_function(p) for p in price_range]
        max_r_price = price_range[revenues.index(max(revenues))]
        max_r_revenue = max(revenues)
        
        # Max Profit
        profits = [profit_function(p) for p in price_range]
        max_p_price = price_range[profits.index(max(profits))]
        ...
        max_b_price = price_range[benefits.index(max(benefits))]
        max_b_benefits = max(benefits)
        
        # Max Social Welfare
        social_welfares = [social_welfare_function(p) for p in price_range]
        max_s_price = price_range[social_welfares.index(max(social_welfares))]
    ...
    def _perform_enhanced_mcda(self, welfare_results):
        ...
        for mode in self.transport_modes:
            ...
            avg_emissions = self._calculate_mean([row['carbon_emissions'] for row in mode_data])
            max_emissions = max(self._calculate_mean([row['carbon_emissions'] for row in self.sp_data 
                                                    if row['Mode_Used'] == m]) for m in self.transport_modes)
            scores['environmental_impact'] = 1 - (avg_emissions / max_emissions)
            
            # Cost effectiveness score
            avg_cost = self._calculate_mean([row['Fare_Paid'] for row in mode_data])
            min_cost = min(self._calculate_mean([row['Fare_Paid'] for row in self.sp_data 
                                               if row['Mode_Used'] == m]) for m in self.transport_modes)
            max_cost = max(self._calculate_mean([row['Fare_Paid'] for row in self.sp_data 
                                               if row['Mode_Used'] == m]) for m in self.transport_modes)
    ...
    def objective_3_analyze_travel_time_and_key_factors(self):
        ...
        for income_group in self.income_groups:
            ...
            mean_vot = self._calculate_mean(time_values)
            
            vot_results[income_group] = {
                'mean_vot': mean_vot,
                'vot_per_hour': mean_vot * 60,  # Convert to per hour
                'median_vot': self._calculate_median(time_values),
        ...
        for mode in self.transport_modes:
            ...
            gc_results[mode] = {
                'mean_gc': self._calculate_mean(gc_values),
                'min_gc': min(gc_values),
                'max_gc': max(gc_values),
                'std_gc': self._calculate_std(gc_values),
                'median_gc': self._calculate_median(gc_values)
            }
